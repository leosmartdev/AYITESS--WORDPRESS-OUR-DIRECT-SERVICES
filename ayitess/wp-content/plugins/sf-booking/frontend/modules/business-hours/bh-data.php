...
if(!empty($breaktime[$weekday])){
	...
	foreach($breaktime[$weekday] as $breakslot){
		...
		$html .= '<li data-breakslot="'.$breakslot.'" data-weekday="'.esc_attr($weekday).'">'.date('h:i a',strtotime($break[0])).' TO '.date('h:i a',strtotime($break[1])).' <span class="working-hours-remove"><i class="fa fa-close"></i></span></li>';
	}
	$html .= '</ul>';
}
...
$success = array(
		...
		'breaktime_html' => $html,
		'suc_message' => esc_html__('Break time addedd successfully.', 'cleanx'),
		);
echo json_encode($success);
