...
$customer_id = (isset($_POST['customer_id'])) ? esc_html($_POST['customer_id']) : '';
...
$userdata = $wpdb->get_row($wpdb->prepare('SELECT * FROM '.$wpdb->users.' WHERE `ID` = %d',$customer_id));
	...
	if(!empty($tokenRow)){
		...
		if( is_array($checkoutDetails) && ($checkoutDetails['ACK'] == 'Success') ) {
				...
				if( is_array($singlePayment) && $singlePayment['ACK'] == 'Success') {
					...
					$customer_id = $tokenRow->provider_id;
...
$userdata = service_finder_getUserInfo($customer_id);
...
$str = "$MERCHANT_KEY|$txnid|$price|$productinfo|$first_name|$user_email|$plan|$customer_id|||||||||$SALT";
...
$customer_id = (isset($_POST['udf2'])) ? esc_html($_POST['udf2']) : '';
...
global $wpdb, $stripe_options, $service_finder_options, $service_finder_Tables;
		...
		try {			
			...
			if ($charge->paid == true && $charge->status == "succeeded") { 
				...
				$makePayment->service_finder_makeJobPostPayment($_POST,$customer->id,$txnid,'stripe');
...
$walletamount = service_finder_get_wallet_amount($customer_id);
...
$makePayment->service_finder_makeJobPostPayment($_POST,'','','wallet');
...
$args = array(
	...
	'amount' => $planprice,
	'action' => 'debit',
	'debit_for' => esc_html__('Purchase Job Post Limit', 'service-finder'),
	'payment_mode' => 'local',
...
$cashbackamount = service_finder_cashback_amount('job-post-limit');
...
update_user_meta($customer_id,'_sf_wallet_amount',$remaining_wallet_amount);
...
$cost = get_user_meta($current_user->ID,'job_applications_cost',true);
$description = get_user_meta($current_user->ID,'job_applications_description',true);
...
delete_user_meta($customer_id, 'job_connect_request');
delete_user_meta($customer_id, 'job_connect_request_status');

update_user_meta($customer_id, 'job_connect_request',$wired_array);
update_user_meta($customer_id, 'job_connect_request_status','pending');
...
service_finder_send_job_post_plan_wiretansfer_mail($customer_id,$invoiceid);

// show messages
$redirect = add_query_arg( array('joblimitplanupdate' => 'wired'), service_finder_get_url_by_shortcode('[service_finder_my_account') );
wp_redirect($redirect);
die;

}

/*Job post plan via wire transfer*/
function service_finder_send_job_post_plan_wiretansfer_mail($customer_id,$invoiceid){
global $wpdb, $service_finder_Tables, $service_finder_options;

$subject = esc_html__('Invoice ID for Job Post Limit via Wire Transfer', 'service-finder');

$userid = $row->provider_id;
$email = service_finder_getCustomerEmail($customer_id);
